...
pub fn start_minecraft_server(detached: bool) {
    let mut bash_cmd: Command = Command::new("bash");
    bash_cmd.arg("/home/mcadmin/minecraft_server/start_minecraft_server.sh");
    if detached { bash_cmd.arg("-d"); }

    println!("{}", style("Starting Minecraft Server...").cyan());
    let err = bash_cmd.exec();
    eprintln!("{}: {}", style("Failed to start minecraft server").red(), err);
}

pub fn stop_minecraft_server() {
    let mut bash_cmd: Command = Command::new("bash");
    bash_cmd.arg("/home/mcadmin/minecraft_server/stop_minecraft_server.sh");
